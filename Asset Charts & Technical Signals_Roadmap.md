...
| Phase 1 · Core MVP                      | In progress | The core workflow is usable across 33 series with chart modes, period aggregation, volume, calibrated core indicators, a separate advanced diagnostic layer, recent views, range measurement, asset ratios, rolling correlations, point-in-time event markers, chain validation, export, and alerts. Full-roadmap acceptance still requires explicit adjustment choices where providers expose multiple bases, richer event types, a true mobile asset drawer, keyboard/assistive-technology verification, and the remaining configurable indicator families. |
