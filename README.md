...
方向分、证据置信度和连续确认次数，且不处于冷却期、日亏损熔断或连续亏损暂停时才开仓；同一时间最多
保留一笔 BTC 仓位。默认最低方向分为 `|score| ≥ 55`，它与置信度分开：方向分描述多空强度，置信度描述
...
Testnet 模式最高允许5倍杠杆，仍受最低方向分、连续确认、单笔名义仓位、冷却期、每日亏损熔断、连续
亏损暂停和单一活动仓位约束。
前端不会读取密钥，D1 也不保存明文。Testnet 平仓后先保留成交均价与手续费假设生成的估算结果；后续周期
通过账户成交明细与资金费收入记录核对真实已实现盈亏、USDT 佣金及持仓期间 BTCUSDT 账户资金费。资金费
接口不关联单笔订单，因此只按本系统仓位持有区间归属；Testnet 账户不应同时持有系统外 BTCUSDT 仓位。对账成功后覆盖为
