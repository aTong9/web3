...
<template>
  <header class="app-topbar">
    ...
    <div class="locale-switch" role="group" :aria-label="t('ui.app.language')">
      <button :class="{ active: locale.value === 'zh' }" @click="setLocale('zh')">{{ t('ui.app.chinese') }}</button>
      <button :class="{ active: locale.value === 'en' }" @click="setLocale('en')">{{ t('ui.app.english') }}</button>
    </div>
    <ThemeToggle />
  </header>
</template>
