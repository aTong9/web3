...
<template>
  <main class="game-income-view">
    ...
    <div class="game-workbench"><aside class="game-directory">
...
<style scoped>
.game-income-view{width:min(var(--content-wide),100%);margin:0 auto;padding:32px var(--page-gutter) 80px}
...
.game-tabs{margin:0 0 20px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.game-tabs button{min-height:104px;padding:12px;text-align:left;border:1px solid var(--border);border-radius:10px;background:var(--surface);color:var(--ink);cursor:pointer;position:relative}.game-tabs button:hover{border-color:color-mix(in srgb,var(--accent) 55%,var(--border))}.game-tabs button:focus-visible{outline:2px solid var(--accent);outline-offset:2px}.game-tabs button.active{border-color:var(--accent);background:var(--accent-soft)}.game-tabs span,.game-tabs small{display:block}.game-tabs span{font-weight:800}.game-tabs small{margin-top:5px;color:var(--muted);font-size:11px}.game-tabs em{display:inline-block;margin-top:10px;padding:3px 6px;border-radius:4px;background:var(--accent-soft);color:var(--accent);font-size:10px;font-style:normal}
...
@media(max-width:620px){.game-income-view{padding:24px var(--page-gutter) 60px}.game-picker-heading{align-items:stretch;flex-direction:column}.game-picker-heading label{width:100%;min-width:0}.game-tabs{display:flex;padding:0 0 8px;overflow-x:auto;scroll-snap-type:x mandatory}.game-tabs button{min-width:180px;min-height:96px;scroll-snap-align:start}.merchant-grid,.route-cards,.boundary-grid,.calculator-grid,.calculator-result,.readiness-grid{grid-template-columns:1fr}.guardrail{padding:18px}.section-nav{top:68px;margin-inline:-6px;box-shadow:none}.route-filter{align-items:stretch;flex-direction:column;gap:4px}.route-filter p{padding:0}.readiness>header{align-items:stretch;flex-direction:column}.readiness-score{text-align:left}.calculator>header{align-items:stretch;flex-direction:column}.calculator-actions{min-width:0;text-align:left}.table-wrap{margin-inline:0;overflow:visible;border:0;background:transparent}table{min-width:0;display:block}thead{display:none}tbody{display:grid;gap:10px}tr{padding:16px;border:1px solid var(--border);border-radius:10px;background:var(--surface);display:block}tbody th{width:auto;padding:0 0 12px;border-bottom:1px solid var(--border);display:block;font-size:15px}td{padding:10px 0 0;border:0;display:grid;grid-template-columns:92px 1fr;gap:10px}td::before{content:attr(data-label);color:var(--muted);font-size:10px;font-weight:800}td small{grid-column:2}}

.game-workbench { display:grid; grid-template-columns:210px minmax(0,1fr); gap:24px; align-items:start; }
.game-directory { min-width:0; position:sticky; top:76px; max-height:calc(100dvh - 92px); overflow:auto; }
.game-reading { min-width:0; }
.game-picker-heading { margin:0 0 14px; }
.game-picker-heading h2 { font-size:20px; }
.game-picker-heading label { display:none; }
.game-tabs { grid-template-columns:1fr; gap:6px; }
.game-tabs button { min-height:70px; padding:12px; }
.game-tabs em { display:none; }
.game-summary { margin-top:0; }
.guardrail { display:block; padding:14px 18px; margin:12px 0; }
.guardrail h2 { font-family:inherit; font-weight:600; font-size:15px; line-height:1.5; margin:0; }
.guardrail > div > span { display:none; }
.guardrail p { margin:6px 0 0; font-size:11px; }
@media(max-width:1100px) { .game-workbench { grid-template-columns:minmax(0,1fr); } .game-directory { position:static; max-height:none; overflow:visible; } .game-tabs { display:flex; overflow:auto; } .game-tabs button { flex:0 0 170px; } .game-picker-heading { display:none; } }
