...
const actionLabel = (action: OptionCandidateAction) => t(`quant.optionAction.${actionKey[action]}`)
...
const addPaperPosition = async (candidate: QuantOptionCandidate) => {
  if (!can('paper.manage') || candidate.price === null || isRecorded(candidate)) return
...
onMounted(async () => {
  ...
  try {
    ...
    const [cloudDashboard, cloudPositions] = await Promise.all([
      quantApi.dashboard(),
      can('paper.manage') ? quantApi.positions(quantClientId) : Promise.resolve([]),
    ])
  ...
  <main class="quant-page">
    <ResearchPageHeader :eyebrow="t('quant.badge')" :title="t('quant.title')" :description="t('quant.intro')">
      ...
      <template #status><section class="strategy-focus" :aria-label="t('quant.configTitle')">
        ...
        <dl>
          <div>
            <dt>{{ t('quant.threshold') }}</dt>
            <dd>{{ dashboard.config.forwardPeThreshold }}x</dd>
          </div>
          <div>
            <dt>{{ t('quant.buffer') }}</dt>
            <dd>{{ dashboard.config.valuationBufferPct }}%</dd>
          </div>
          <div>
            <dt>{{ t('quant.risk') }}</dt>
            <dd>{{ dashboard.config.maximumPositionRiskPct }}%</dd>
          </div>
          <div>
    ...
    <section class="summary-strip">
      <div>
        ...
        ><strong>{{ dashboard.summary.buyCandidates }}</strong>
      </div>
      <div>
        <span>{{ t('quant.summary.sell') }}</span
        ><strong>{{ dashboard.summary.sellCandidates }}</strong>
      </div>
      <div>
        <span>{{ t('quant.summary.calls') }}</span
        ><strong>{{ dashboard.summary.optionLongCallCandidates }}</strong>
      </div>
      <div>
        <span>{{ t('quant.summary.exits') }}</span
        ><strong>{{ dashboard.summary.optionExitCandidates }}</strong>
      </div>
    </section>

    <section class="section-heading">
      <div>
        <h2>{{ t('quant.assetTitle') }}</h2>
        <p>{{ t('quant.assetHint') }}</p>
      </div>
    ...
    <section class="asset-table-wrap">
      <div class="asset-head">
        <span>{{ t('quant.columns.asset') }}</span
        ><span>{{ t('quant.columns.price') }}</span> <span>{{ t('quant.columns.changes') }}</span
        ><span>{{ t('quant.columns.score') }}</span> <span>{{ t('quant.columns.evidence') }}</span
        ><span>{{ t('quant.columns.signal') }}</span>
      </div>
      <details v-for="asset in filteredAssets" :key="asset.id" class="asset-row">
        <summary>
          <span class="asset-name"
            ><b>{{ asset.name }}</b
            ><small>{{ modelLabel(asset.modelSource) }} · {{ asset.date }}</small></span
          >
          <span>{{ formatValue(asset) }}</span>
          <span
            >{{ formatChange(asset, asset.changes.week) }} /
            {{ formatChange(asset, asset.changes.month) }}</span
          >
          <strong :class="asset.score >= 0 ? 'positive' : 'negative'"
            >{{ asset.score > 0 ? '+' : '' }}{{ asset.score }}</strong
          >
          <span class="evidence"
            ><i :style="{ width: `${asset.evidenceScore}%` }"></i
            ><b>{{ asset.evidenceScore }}</b></span
          >
        ...
        <div class="asset-detail">
          <section>
            <b>{{ t('quant.reasons') }}</b>
            <ul>
              <li v-for="reason in asset.reasons" :key="reason">{{ reason }}</li>
            </ul>
          </section>
          <section>
            <b>{{ t('quant.risks') }}</b>
            <ul>
              <li v-for="risk in asset.risks" :key="risk">{{ risk }}</li>
    ...
    <section class="option-grid">
      <article v-for="candidate in dashboard.options" :key="candidate.symbol" class="option-card">
        <header>
          <div>
            <span>{{ t('quant.rank', { rank: candidate.marketCapRank }) }}</span>
          ...
          <div class="option-recommendation">
            ...
            <small
              >{{ candidate.template.dteRange.min }}–{{
                candidate.template.dteRange.max
              }}
              DTE</small
            >
          </div>
        ...
        <div class="option-metrics">
          <div>
            <span>Forward PE</span><strong>{{ candidate.forwardPe?.toFixed(2) ?? '—' }}x</strong>
          </div>
          <div>
            <span>{{
              t('quant.peGap', { value: formatPct(candidate.discountToThresholdPct) })
            }}</span
            ><strong>${{ candidate.price?.toFixed(2) ?? '—' }}</strong>
          </div>
          <div>
        ...
        <section class="earnings-panel">
          <header>
            ...
            <em :class="candidate.earningsWindow">{{
              earningsWindowLabel(candidate.earningsWindow)
            }}</em>
          </header>
          <dl>
            <div>
            ...
            <div>
              ...
              <dd
                v-if="candidate.earnings.lastResultReliable"
                :class="(candidate.earnings.lastSurprisePct ?? 0) >= 0 ? 'positive' : 'negative'"
              >
            ...
            <div>
              <dt>
                {{
                  t('quant.earnings.nextEstimate', {
                    quarter: candidate.earnings.nextFiscalQuarter ?? '—',
                  })
                }}
              </dt>
            ...
            <div>
              ...
              <dd>
                {{ candidate.earnings.revisionsUp ?? '—' }} /
                {{ candidate.earnings.revisionsDown ?? '—' }}
              </dd>
            </div>
            <div>
              <dt>{{ t('quant.earnings.range') }}</dt>
              <dd>
                {{ candidate.earnings.nextLowEps?.toFixed(2) ?? '—' }}–{{
                  candidate.earnings.nextHighEps?.toFixed(2) ?? '—'
                }}
              </dd>
            </div>
            <div>
        ...
        <footer>
          <span class="execution">{{
            candidate.executable ? t('quant.executable') : t('quant.notExecutable')
          }}</span>
          <button
            :disabled="
              !can('paper.manage') ||
              candidate.price === null ||
              isRecorded(candidate) ||
              busyPositionId === candidate.symbol
            "
            @click="addPaperPosition(candidate)"
          >
            {{
              !can('paper.manage')
                ? t('quant.permissionRequired')
                : isRecorded(candidate)
                  ? t('quant.recorded')
                  : t('quant.addPaper')
            }}
    ...
    <section class="paper-section">
      ...
      <article v-for="position in paperPositions" :key="position.id" class="paper-row">
        ...
        <div>
          ...
          ><b>{{ formatDate(position.openedAt) }}</b>
        </div>
        <div>
          <small>{{ t('quant.paperEntry') }}</small
          ><b>${{ position.entryUnderlyingPrice.toFixed(2) }}</b>
        </div>
        <div>
          <small>{{ t('quant.paperReturn') }}</small
          ><b :class="(paperReturn(position) ?? 0) >= 0 ? 'positive' : 'negative'">{{
            formatPct(paperReturn(position))
          }}</b>
        </div>
        <button
          v-if="position.status === 'open'"
          :disabled="!can('paper.manage') || busyPositionId === position.id"
          @click="closePaperPosition(position)"
        >
          {{ t('quant.paperClose') }}
        </button>
        <button
          ...
          class="remove-record"
          :disabled="!can('paper.manage') || busyPositionId === position.id"
          @click="removePaperPosition(position)"
        >
          {{ t('quant.paperRemove') }}
        </button>
      </article>
    ...
    <details class="methodology">
      ...
      <ul>
        <li v-for="limitation in dashboard.limitations" :key="limitation">{{ limitation }}</li>
      </ul>
    </details>
...
.strategy-focus header span {
  color: var(--muted);
  font-size: 10px;
  font-weight: 700;
  letter-spacing: 0.08em;
}
.strategy-focus header b {
  color: var(--accent);
  font:
    500 24px Georgia,
    serif;
...
.asset-row summary {
  ...
  grid-template-columns:
    minmax(190px, 1.3fr) minmax(115px, 0.8fr) minmax(150px, 1fr) 70px minmax(110px, 0.8fr)
    minmax(110px, 0.8fr);
  gap: 14px;
  align-items: center;
}
...
.asset-row summary::-webkit-details-marker {
  display: none;
}
...
.asset-row summary > span {
  font-size: 11px;
  font-variant-numeric: tabular-nums;
}
.asset-row summary > strong {
  font-size: 16px;
  font-variant-numeric: tabular-nums;
}
.positive {
...
.asset-row em.buy,
.asset-row em.accumulate,
.option-card em.long-call-candidate {
  background: var(--danger-soft);
  color: var(--positive);
}
.asset-row em.sell,
.asset-row em.reduce,
.option-card em.exit-long-call,
.option-card em.avoid {
  background: var(--accent-soft);
  color: var(--negative);
}
.asset-detail {
...
.option-card header span,
.option-card header small {
  display: block;
  color: var(--muted);
  font-size: 8px;
}
...
.option-recommendation em.bullish {
  background: var(--danger-soft);
  color: var(--positive);
}
.option-recommendation em.bearish {
  background: var(--accent-soft);
  color: var(--negative);
}
.option-recommendation em.event-risk {
  color: var(--warning);
}
.option-recommendation b {
  font-size: 11px;
}
.option-recommendation small {
  color: var(--muted);
  font-size: 8px;
  font-variant-numeric: tabular-nums;
}
...
.earnings-panel > header b {
  font-size: 10px;
}
.earnings-panel > header em {
  padding: 4px 6px;
  border-radius: 4px;
  background: var(--surface);
  color: var(--muted);
...
.earnings-panel > header em.pre-earnings {
  color: var(--warning);
}
.earnings-panel > header em.post-earnings {
  color: var(--accent);
}
.earnings-panel dl {
  margin: 10px 0 0;
...
.option-detail b:not(:first-child) {
  display: block;
  margin-top: 12px;
}
.option-card > footer {
...
.paper-row {
  ...
  border-radius: 8px;
  display: grid;
  grid-template-columns: minmax(140px, 1fr) repeat(3, minmax(100px, 0.7fr)) auto;
  gap: 16px;
  align-items: center;
}
.paper-row + .paper-row {
...
.paper-row div:first-child span,
.paper-row small {
  color: var(--muted);
  font-size: 8px;
}
...
@media (max-width: 1050px) {
  ...
  .asset-row summary {
    grid-template-columns: minmax(170px, 1.2fr) 1fr 1fr 55px 90px;
  }
  .asset-row summary > em {
    grid-column: 2 / -1;
  }
}
