...
CREATE TABLE technical_alert_rules (
  ...
  compare_asset_id TEXT,
  compare_asset_name TEXT,
  ...
  CHECK (
    ...
    OR (compare_asset_id IS NOT NULL AND compare_asset_name IS NOT NULL)
  ),
...
INSERT INTO technical_alert_rules (
  ...
  condition, threshold, horizon, minimum_confidence, require_resonance, enabled,
...
CREATE UNIQUE INDEX idx_technical_alert_rules_unique
  ON technical_alert_rules(
    user_id,
    asset_id,
    condition,
    COALESCE(compare_asset_id, '')
  );
